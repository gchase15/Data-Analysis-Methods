...
%          imshow(blurspectro)
    ...
    feature = 48; %must be <= the number of samples per band
    
    [result,w,U,S,V,threshold,sortSound1,sortSound2,sortSound3,sortSound4]...
        =Test1_trainer(band1_wave,band2_wave,band3_wave,band4_wave,feature);
...
eng10perc=sum(sig(1:5))/sum(sig)
eng25perc=sum(sig(1:12))/sum(sig)
eng50perc=sum(sig(1:24))/sum(sig)
eng=sum(sig(1:44))/sum(sig)
...
xband1 = V(1:12,1:16); xband2 = V(13:24,1:16);
xband3 = V(25:36,1:16); xband4 = V(37:48,1:16);

xtrain = [xband1(q1(1:7),:); xband2(q2(1:7),:); xband3(q2(1:7),:); xband4(q4(1:7),:)];
xtest = [xband1(q1(8:end),:); xband2(q2(8:end),:); xband3(q3(8:end),:); xband4(q4(8:end),:)];
ctrain = [ones(7,1); 2*ones(7,1); 3*ones(7,1); 4*ones(7,1);];
...
success1lda = sum(band1prelda(:) == 1)/5000
success2lda = sum(band2prelda(:) == 2)/5000
success3lda = sum(band3prelda(:) == 3)/5000
success4lda = sum(band4prelda(:) == 4)/5000

success1nb = sum(band1prenb(:) == 1)/5000
success2nb = sum(band2prenb(:) == 2)/5000
success3nb = sum(band3prenb(:) == 3)/5000
success4nb = sum(band4prenb(:) == 4)/5000

success1tree = sum(band1pretree(:) == 1)/5000
success2tree = sum(band2pretree(:) == 2)/5000
success3tree = sum(band3pretree(:) == 3)/5000
...
figure(6)
subplot(1,4,1)
histogram(band1prelda,4)
axis([1 4 0 5000]), xticks([1.2 2 3 3.8]),xticklabels({'AC/DC', 'Third Day', 'John Williams', 'Army Songs'})
set(gca,'YTick',[1000 2000 3000 4000 5000], 'Fontsize',[12])
subplot(1,4,2)
histogram(band2prelda,4), axis([1 4 0 5000])
xticks([1.2 2 3 3.8]),xticklabels({'AC/DC', 'Third Day', 'John Williams', 'Army Songs'})
set(gca,'YTick',[1000 2000 3000 4000 5000], 'Fontsize',[12])
subplot(1,4,3)
histogram(band3prelda,4), axis([1 4 0 5000])
xticks([1.2 2 3 3.8]),xticklabels({'AC/DC', 'Third Day', 'John Williams', 'Army Songs'})
set(gca,'YTick',[1000 2000 3000 4000 5000], 'Fontsize',[12])
subplot(1,4,4)
histogram(band4prelda,4), axis([1 4 0 5000])
xticks([1.2 2 3 3.8]),xticklabels({'AC/DC', 'Third Day', 'John Williams', 'Army Songs'})
set(gca,'YTick',[1000 2000 3000 4000 5000], 'Fontsize',[12])
